...
{	
	...
	stats = malloc(sizeof(int)*degrees->allrecs);
	memset(stats,-1,degrees->allrecs*sizeof(int));
	
	for(i=0;i<degrees->size;i++){
		b = degrees->ht[i];
		for(j=0;j<b->nrecs;j++){
			r = b->cells[j];			
			index = sortDegrees(stats,degrees->allrecs,r->id);
			stats[index] = r->id;
		}
	}
	...
	for(i=0; i<degrees->allrecs; i++){
		r = lookupNode(stats[i],degrees);
...
{	
	...
	memmove(&(stats[m+1]),&(stats[m]),sizeof(int)*(size-m-1));
	return m;
}
/******************************************************************************/
