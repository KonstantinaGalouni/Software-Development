...
/******************************************************************************/
int removeEdge(int start,int end, Graph *g)
{
	Record *r;
	
	if((r = lookupNode(start,g)) == NULL){
		printf("Node id %d does not exist in graph\n",start);
		return 0;
	}
	
	Edge *previous,*e = r->edges;
	int found = 0;
	...
	if(found == 1){
		...
		//deleting edge from start to end
		free(e->props->properties[0]);
		free(e->props->properties);
		free(e->props);
		free(e);
