...
Matches* matchSuggestion(Record* n, int k, int h, int x, int limit, Graph* g)
{
	...
	int i,j,curDist,insert,index1,index2,index=0,tomh=0,enwsh=0;
	double score=0.0;
	...
	Edge *e;
	Properties *prop;
	Graph *location, *work, *study, *suggestion, *visited;
	...
	// create graph with nodes which have up to h distance from n, but not its friends

	suggestion = create_Graph(2,3);
	visited = create_Graph(2,3);
	create_list(&queue);
	e = n->edges;
	while(e!=NULL){
		prop = create_Properties(1);
		setIntegerProperty(1, 0, prop);
	...
	for(i=0; i<suggestion->size; i++){
		...
		for(j=0; j<b->nrecs; j++){
			...
			if(insert == 1){			//try inserting record in matches
				score = tomh/(double)enwsh;
				insert = sortMatches(matches ,score, limit);
				if(insert < limit){		//insert if score is among top-limit
...
void findTrends(int k, Graph *g, char **womenTrends, char **menTrends)
{
	...
	for(i=0;i<tags->size;i++){
		...
		for(j=0;j<b->nrecs;j++){
			...
			max = find_maxCC(g,men,tag,0);		//max cc of men for this tag
			...
			max = find_maxCC(g,women,tag,1);	//max cc of women for this tag		
...
{	
	...
	// create graph with records interested in tag->id
	...
	p = maxCC(tagCC);	// find max cc in graph tagcc
